...
(defn day-file-stream [day]
  (jio/reader (jio/resource (str "bristol/2019/csv/tabula-" (string/lower-case day) "19.csv"))))

(defn read-schedules-dates []
  (into {} (map (fn [day] [(keyword (string/lower-case day))
                           (parse-schedule-file (day-file-stream day))])
...
(defn read-schedules []
  (build-schedules schedule-schemas (read-schedules-dates) waste-type-cycles))

(comment
  (:thursday-w (build-schedules schedule-schemas (read-schedules-dates)))
  (:monday (read-schedules-dates))
